Solve a bug reported in a Slack thread using a TDD approach.

Slack message link: $ARGUMENTS

Steps:

1. **Parse the Slack link**
   Extract the channel ID and message timestamp from the provided Slack message URL.
   - URL format: `https://<workspace>.slack.com/archives/<channel_id>/p<timestamp>`
   - Convert the `p<timestamp>` to Slack's `ts` format by inserting a `.` before the last 6 digits (e.g., `p1234567890123456` → `1234567890.123456`).
   - If the link cannot be parsed, ask the user to provide the channel ID and thread timestamp manually.

2. **Read the full Slack thread**
   Use `mcp__claude_ai_Slack__slack_read_thread` with the extracted channel and timestamp to read the entire bug report thread.
   - If the thread is long, paginate to capture all messages.
   - Extract: the original bug report, any screenshots or error logs shared, follow-up discussion, and any attempted fixes mentioned.
   - If Slack reading fails, ask the user to paste the bug report content manually.
