const THRESHOLD_SECONDS = 60; // in case you are changing this, change the cron time too.

const LOCK_TTL_IN_SECONDS = 10;

const SCHEDULE_JOB_MAX_RETRIES = 3;

const SCHEDULE_JOB_BASE_DELAY_MS = 500;

const SCHEDULE_JOB_MAX_JITTER_MS = 150;

const EXTERNAL_SERVICE_TIMEOUT_IN_MS = 10000;

const CONSUMER_RETRY_MAX_RETRIES = 3;

const CONSUMER_RETRY_BASE_DELAY_MS = 500;

const CONSUMER_RETRY_MAX_JITTER_MS = 150;

const PRIMARY_PREFERRED_READ = 'primaryPreferred';

const RECONCILE_CRON_SCHEDULE = '*/5 * * * *';

const RECONCILE_STALE_GRACE_SECONDS = 60;

const RECONCILE_MAX_STALENESS_SECONDS = 6 * 60 * 60;

export {
  THRESHOLD_SECONDS,
  LOCK_TTL_IN_SECONDS,
  SCHEDULE_JOB_MAX_RETRIES,
  SCHEDULE_JOB_BASE_DELAY_MS,
  SCHEDULE_JOB_MAX_JITTER_MS,
  EXTERNAL_SERVICE_TIMEOUT_IN_MS,
  CONSUMER_RETRY_MAX_RETRIES,
  CONSUMER_RETRY_BASE_DELAY_MS,
  CONSUMER_RETRY_MAX_JITTER_MS,
  PRIMARY_PREFERRED_READ,
  RECONCILE_CRON_SCHEDULE,
  RECONCILE_STALE_GRACE_SECONDS,
  RECONCILE_MAX_STALENESS_SECONDS,
};
